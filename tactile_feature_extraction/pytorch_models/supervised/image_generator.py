...
class PhotoDataset_combinexy(torch.utils.data.Dataset):
    def __init__(
            self,
            photos_dir,
            labels_dir,
    ...
    def __getitem__(self, idx):
        ...
        labels = {'Fxy': math.sqrt(label[0]**2+ label[1]**2), 'Fz': label[2]}
        sample = {'images': torch.tensor(frames_data, dtype=torch.float32), 'labels': labels}
        return sample
...
import math
class PhotoDataset_ConvLstm_combinexy(torch.utils.data.Dataset):
    def __init__(
            self,
            photos_dir,
            labels_dir,
...
class PhotoDataset_Seq2Seq(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        frames_data, label = self.samples[idx]
        # shape = (3, n_frames)
        labels = {'Fx': label[:, 0].reshape(-1), 'Fy': label[:, 1].reshape(-1), 'Fz': label[:, 2].reshape(-1)}

        sample = {'images': torch.tensor(frames_data, dtype=torch.float32), 'labels': labels}
        return sample

class PhotoDataset_Seq2Seq_combinexy(torch.utils.data.Dataset):
    def __init__(
            self,
            photos_dir,
            labels_dir,
    ...
    def __getitem__(self, idx):
        frames_data, label = self.samples[idx]
        # shape = (3, n_frames)
        labels = {'Fxy': np.sqrt(label[:, 0].reshape(-1)**2 + label[:, 1].reshape(-1)**2), 'Fz': label[:, 2].reshape(-1)}

...
class SimpleSensor:
    def __init__(
        ...
        self.cam.set(cv2.CAP_PROP_AUTO_EXPOSURE, auto_exposure_mode)
        self.cam.set(3, 640)
        self.cam.set(4, 480)
        self.cam.set(cv2.CAP_PROP_EXPOSURE, exposure)
        self.cam.set(cv2.CAP_PROP_EXPOSURE, exposure)
        self.cam.set(cv2.CAP_PROP_BRIGHTNESS, brightness)
        for _ in range(5):
...
def list_camera_sources():
    ...
    working_ports = []
    available_ports = []
    while len(non_working_ports) < 14:  # if there are more than 5 non working ports stop the testing.

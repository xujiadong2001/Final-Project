...
def create_model(
    ...
    elif model_params['model_type'] == 'conv_lstm_attention':
...
class ConvGRUAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        conv_output = self.conv_model(conv_input)
        gru_input = conv_output.view(batch_size, timesteps, -1)
        output,hidden = self.gru(gru_input) # hidden [num_layers, batch_size, hidden_dim]
        context_vector, attention_weights = self.attention(hidden[-1], output) # hidden[-1] [batch_size, hidden_dim]
...
class TimeAttention(nn.Module):
    def __init__(
        ...
        self.feature_update = nn.Linear(512, 128)
    ...
    def forward(self, x):
        ...
        for t in range(timesteps):
            ...
            weighted_input = attention_weights * temp_input # [batch_size, 128]
...
'''Traceback (most recent call last):
  File "Main_Code/model_learning/launch_training_improved_photos.py", line 185, in <module>
    launch()
  File "Main_Code/model_learning/launch_training_improved_photos.py", line 139, in launch
    train_model_w_metrics(
  File "/root/autodl-tmp/Final-Project/Main_Code/pytorch_models/supervised/train_model_w_metrics.py", line 211, in train_model_w_metrics
    val_epoch_loss, val_epoch_acc, val_acc_df, val_err_df, val_pred_df, val_targ_df = run_epoch(
  File "/root/autodl-tmp/Final-Project/Main_Code/pytorch_models/supervised/train_model_w_metrics.py", line 123, in run_epoch
  ...
  File "/root/autodl-tmp/Final-Project/Main_Code/pytorch_models/supervised/models.py", line 1905, in forward
